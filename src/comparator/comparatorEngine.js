...
class ComparatorEngine {
  ...
  static _compareJsonSchema(oldSchema, newSchema, currentPath, breaking, nonBreaking, opts) {
    ...
    if (opts.treatRemovedRequiredAsBreaking) {
      ...
      for (const field of oldRequired) {
        if (!newRequired.has(field)) {
          ...
          // Only report here when the field still exists but lost its "required"
          // status. If the field was removed entirely, the "Properties" check
          // below already reports a FIELD_REMOVED for it — reporting it again
          // here would duplicate the same breaking change.
          if (stillExists) {
            breaking.push({
              type: 'REQUIRED_REMOVED',
              path: `${pathPrefix}${field}`,
              message: `Field "${field}" was required and is now optional`
            });
          }
        }
      }
    ...
    // New properties are non-breaking
    ...
    // 4. Enum values
    if (opts.treatEnumRemovalAsBreaking && oldSchema.enum && newSchema.enum) {
